...
class DispatchBot extends ActivityHandler {
        ...
        this.onMembersAdded(async (context, next) => {
            ...
            for (let member of membersAdded) {
                if (member.id !== context.activity.recipient.id) {
                    await context.sendActivity(`Welcome to Dispatch bot ${member.name}. ${welcomeText}`); // UNDEFINED MEMBER.NAME
                }
            }
    ...
    async processWeather(context, luisResult) {
        ...
        await context.sendActivity(`ProcessWeather intents detected:\n\n  ${luisResult.intents.map((intentObj) => intentObj.intent).join('\n\n')}.`);
